...
export async function GET(
    ...
    try {
        ...
        console.error("Error fetching briefing tasks:", error);
        return NextResponse.json({ error: "Failed to fetch briefing tasks." }, { status: 500 });
    }
}
...
export async function POST(
    ...
    try {
        ...
        // Parse title and desc from userInstruction
        ...
        const topicDescription = lines.find((l: string) => l.startsWith("Beschreibung: "))?.replace("Beschreibung: ", "") || "";
        ...
        const combinedHint = customHint
            ? (weekPlan.globalHints ? `${weekPlan.globalHints}\nSpezifischer Hinweis für dieses Briefing: ${customHint}` : customHint)
            : weekPlan.globalHints;
        ...
        await enqueueTask('generate-briefing', {
            ...
            globalHints: combinedHint
        });

        return NextResponse.json({ success: true, taskId: dbTask.id });

    } catch (error) {
        console.error("Error regenerating briefing task:", error);
